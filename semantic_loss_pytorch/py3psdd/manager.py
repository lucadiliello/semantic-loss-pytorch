...
class SddManager:
    ...
    def _canonical_elements(self,elements):
        ...
        # the following line had to be removed and substituted with __lt__ in sddnode class
        # cmpf = lambda x,y: cmp(x[0].id,y[0].id)
        # print(elements[0][0].id)
        # print(type(elements[0][0].id))
        elf = lambda x: tuple(sorted(x))
        return elf(elements)
